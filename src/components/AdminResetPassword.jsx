...
export default function AdminResetPassword({ onGoAdmin, onBackHome }) {
  ...
  async function handleSubmit(event) {
    ...
    if (!token) {
      setError('Reset token is missing. Please use the full reset link from your email.');
      return;
    }
    ...
    try {
      ...
      setStatus(result.message || 'Password updated successfully. Redirecting to sign in...');
      setPassword('');
      setConfirmPassword('');
    } catch (err) {
    ...
    <section className="mx-auto w-full max-w-3xl space-y-5">
      ...
      <form className={`${ui.card} mx-auto w-full max-w-xl space-y-4 p-4 sm:p-5`} onSubmit={handleSubmit}>
        <div className="space-y-1 text-center">
          <h2 className="text-2xl font-bold tracking-tight text-emerald-950">Set new password</h2>
          <p className={ui.note}>Use at least 8 characters and keep your password secure.</p>
        </div>
        ...
        <div className="flex justify-center pt-1">
          <button type="submit" className={`${ui.buttonPrimary} min-w-36`} disabled={loading || !token}>
        ...
        {!token ? <p className={ui.error}>Reset token not found in URL.</p> : null}
